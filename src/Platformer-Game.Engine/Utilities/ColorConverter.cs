...
namespace PlatformerGame.Engine.Utilities
{
    public class ColorConverter
    {
        public static Color Convert(string rgbSource)
        {
            ...
            byte r, g, b, a = 255;
            bool successR = byte.TryParse(rgbSource.Substring(offset + 0, 2), NumberStyles.HexNumber, null, out r);
            bool successG = byte.TryParse(rgbSource.Substring(offset + 2, 2), NumberStyles.HexNumber, null, out g);
            bool successB = byte.TryParse(rgbSource.Substring(offset + 4, 2), NumberStyles.HexNumber, null, out b);

            bool successA = true;
            if (sourceLength == 8)
                successA = byte.TryParse(rgbSource.Substring(offset + 6, 2), NumberStyles.HexNumber, null, out a);
